...
class GPUCacheManager:
    ...
    def __init__(self, cache_config, logger=None):
        ...
        self.cache_mode = cache_config.get('cache_mode', 'partial')  # 'full', 'partial', 'adaptive'
        self.cache_ratio = cache_config.get('cache_ratio', 0.7)
        self.gpu_devices = cache_config.get('gpu_devices', [0, 1])
        self.max_cache_memory = cache_config.get('max_cache_memory_gb', 60) * (1024**3)  # Convert to bytes
    ...
    def _initialize_cache_memory_limits(self):
        ...
        self.logger.info(f"Cache memory limit per GPU: {self.memory_per_gpu / (1024**3):.2f} GB")
        ...
        for gpu_id in self.gpu_devices:
            try:
                ...
                self.logger.info(f"GPU {gpu_id} - Total: {total_memory/(1024**3):.2f}GB, "
    ...
    def should_cache_sample(self, idx, sample_size_estimate=None):
        ...
        if sample_size_estimate:
            ...
            if self.cache_memory_usage[gpu_id] + sample_size_estimate > self.memory_per_gpu:
                return False
            ...
            with torch.cuda.device(gpu_id):
                ...
                for key, value in sample_data.items():
                    if isinstance(value, torch.Tensor):
                        ...
                        sample_memory += cached_tensor.nelement() * cached_tensor.element_size()
    ...
    def evict_lru_samples(self, target_memory_per_gpu):
        ...
        for gpu_id in self.gpu_devices:
            ...
            gpu_samples = [(idx, info) for idx, info in self.cache.items() 
    ...
    def get_cache_stats(self):
        ...
        return {
            ...
            'memory_usage_gb': {gpu: usage / (1024**3) for gpu, usage in self.cache_memory_usage.items()},
...
class CachedDatasetWrapper:
    ...
    def preload_cache(self, indices=None, batch_size=32):
        ...
        self.cache_manager.logger.info(f"Preloading {len(indices)} samples into cache...")
        ...
        for i in range(0, len(indices), batch_size):
            ...
            for idx in batch_indices:
                try:
                    ...
                    self.cache_manager.logger.warning(f"Failed to preload sample {idx}: {e}")
        ...
        with EventStorage() as self.storage, ExceptionWriter():
            ...
            for self.epoch in range(self.start_epoch, self.max_epoch):
                ...
                for (
                    ...
                    if (self.cache_manager.enabled and 
                        self.comm_info["iter"] % 100 == 0 and 
                        comm.is_main_process()):
                        stats = self.cache_manager.get_cache_stats()
                        self.logger.info(f"Iter {self.comm_info['iter']}: "
        ...
        try:
            ...
            for gpu_id in self.cache_manager.gpu_devices:
                ...
                if reserved > total * 0.9:  # 90% threshold
                    ...
                    self.logger.info(f"Evicted cache on GPU {gpu_id} due to high memory usage")
        except Exception as e:
    ...
    def build_val_loader(self):
        ...
        if self.cfg.evaluate:
            ...
            val_loader = torch.utils.data.DataLoader(
                ...
                shuffle=False,
                num_workers=2 if self.cache_manager.enabled else self.cfg.num_worker_per_gpu,

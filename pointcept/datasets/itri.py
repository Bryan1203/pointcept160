...
class ItriDataset(DefaultDataset):
    def __init__(
        ...
        **kwargs,
        ...
        if domain_names is None:
            self.domain_names = [
                os.path.basename(root.rstrip("/")) for root in self.data_roots
            ]
        else:
        ...
        if self.is_multi_domain:
            assert len(self.data_roots) == len(
                self.domain_names
    ...
    def _create_split_files(self, data_root):
        ...
        print(
            f"Created splits for {domain_name} from {len(scan_files)} scans: {[(k, len(v)) for k, v in splits.items()]}"
        )
        ...
        if not self.is_multi_domain:
            ...
            if os.path.exists(split_file):
                ...
                if self.concat_scans > 1:
                    ...
                    for i in range(0, len(filenames), self.concat_scans):
                        ...
                        if i + self.concat_scans <= len(filenames):
                            ...
                            for j in range(self.concat_scans):
                                scan_group.append(
                                    os.path.join(self.data_roots[0], filenames[i + j])
                                )
                    ...
                    return [
                        os.path.join(self.data_roots[0], fname) for fname in filenames
                    ]
            else:
                return []
                ...
                if self.concat_scans > 1:
                    ...
                    for i in range(0, len(filenames), self.concat_scans):
                        ...
                        if i + self.concat_scans <= len(filenames):
                            ...
                            for j in range(self.concat_scans):
                                scan_group.append(
                                    os.path.join(data_root, filenames[i + j])
                                )
    ...
    def get_data(self, idx):
        ...
        if not self.is_multi_domain:
            ...
            if self.concat_scans > 1:
                ...
                return self._load_and_concatenate_scans(
                    scan_paths, name, split, data_root, domain_idx, domain_name
                )
            else:
                ...
                return self._load_single_scan(
                    data_path, name, split, data_root, domain_idx, domain_name
                )
        else:
            # Multi-domain case
    ...
    def _load_single_scan(
        self, data_path, name, split, data_root, domain_idx, domain_name
    ):
        """Load a single scan"""
        # Load scan data
        ...
        segment = np.vectorize(self.learning_map.__getitem__)(orig_labels).astype(
            np.int32
        )
        ...
        if os.path.exists(pose_file):
            ...
            if pose_idx is not None:
                ...
                print(
                    f"  Single scan {scan_num}: orig center [{orig_center[0]:.2f}, {orig_center[1]:.2f}, {orig_center[2]:.2f}] -> local center [{new_center[0]:.2f}, {new_center[1]:.2f}, {new_center[2]:.2f}]"
                )
    ...
    def _load_and_concatenate_scans(
        self, scan_paths, name, split, data_root, domain_idx, domain_name
    ):
        ...
        if os.path.exists(pose_file):
            ...
            if reference_pose_idx is not None:
                ...
                print(
                    f"  Using scan {first_scan_num} as reference origin for concatenation"
                )
                print(
                    f"  Reference pose - R shape: {reference_R.shape}, t: [{reference_t[0]:.2f}, {reference_t[1]:.2f}, {reference_t[2]:.2f}]"
                )
            else:
        ...
        for scan_idx, scan_path in enumerate(scan_paths):
            ...
            if (
                poses is not None
                and reference_R is not None
                and reference_t is not None
            ):
                # Store original center for debugging
                orig_center = np.mean(coord, axis=0)
                ...
                if scan_idx == 0:
                    print(
                        f"    First scan {scan_num}: orig center [{orig_center[0]:.2f}, {orig_center[1]:.2f}, {orig_center[2]:.2f}] -> ref center [{new_center[0]:.2f}, {new_center[1]:.2f}, {new_center[2]:.2f}]"
                    )
                    print(
                        f"    Note: First scan should be close to origin after transformation"
                    )
                else:
                    print(
                        f"    Scan {scan_num}: orig center [{orig_center[0]:.2f}, {orig_center[1]:.2f}, {orig_center[2]:.2f}] -> ref center [{new_center[0]:.2f}, {new_center[1]:.2f}, {new_center[2]:.2f}]"
                    )
            else:
                scan_num = int(os.path.splitext(os.path.basename(scan_path))[0])
                print(
                    f"    Warning: No pose transformation applied for scan {scan_num}"
                )

            instance = np.ones(coord.shape[0], dtype=np.int32) * (
                scan_idx + 1
            )  # Start from 1
        ...
        data_dict = {
            ...
            "scan_boundaries": [0]
            + [
                sum(len(coord) for coord in all_coords[: i + 1])
                for i in range(len(all_coords))
            ],  # Point boundaries for each scan
        }
    ...
    def get_data_name(self, idx):
        ...
        if not self.is_multi_domain:
            ...
            if self.concat_scans > 1:
                scan_paths = self.data_list[idx % len(self.data_list)]
                scan_nums = [
                    os.path.splitext(os.path.basename(path))[0] for path in scan_paths
                ]
    ...
    def save_individual_predictions(
        self, predictions, data_dict, output_dir, save_format="npy"
    ):
        """
